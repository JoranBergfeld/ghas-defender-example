...
| 4 | Secret Scanning — pushed token | `src/backend/src/main/resources/application-local.yml` | Canonical AWS EXAMPLE access key + secret pair generated by `scripts/seed-secret.sh` | GitHub Secret Scanning patterns `Amazon AWS Access Key ID` and `Amazon AWS Secret Access Key` | Push protection rejects `git push` before the commit reaches GitHub |
| 5 | Dependency — Dependabot | `src/backend/pom.xml`; `src/frontend/package.json` | Pin `commons-fileupload:commons-fileupload:1.3.1` and `axios:0.21.0` | Dependabot alerts for CVE-2016-1000031 and CVE-2021-3749 | Dependabot alert and security update PR; dependency-review blocks PRs when configured by Plan 6 |
