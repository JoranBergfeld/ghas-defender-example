...
| 4 | Secret Scanning — pushed token | `src/backend/src/main/resources/application-local.yml` | Random `sk_live_<24 base62 chars>` Stripe live secret key generated by `scripts/seed-secret.sh` | GitHub Secret Scanning pattern `Stripe API Key` | Push protection rejects `git push` before the commit reaches GitHub |
